...
#  cc9fce6016db076497454f9352e55b4758ccc07c/six.py?at=default#cl-632
...
def import_module_by_code(mod_code, mod_name, sys_add=True, sys_use=True):
    ...
    @param sys_use: whether use an existing module with the same name in
    |sys.modules|,
...
def getattr_chain(obj, attr_chain, sep='.'):
    """Get the last attribute of a specified chain of attributes from a
    specified object.
...
def load_obj(
    ...
    """Load an object from a module (specified by module name in Python
    namespace)
...
def load_obj_http(
    ...
    @param uri: specify the remote module file's location and which attribute
    object to load.
    ...
    if not mod_name:
        ...
        if not mod_name:
            raise ValueError(
                'Module name can not be inferred from the URI.\n URI is |%s|'
                % uri)
...
def load_obj_local_or_remote(
    ...
    @param mod_attr_sep: see func |load_obj| or |load_obj_http|'s same name
    arg.

    @param attr_chain_sep: see func |load_obj| or |load_obj_http|'s same name
    arg.
